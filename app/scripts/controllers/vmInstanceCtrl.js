define(['TM','echarts','eChartTheme',
	...
	var Ctrl = {
		init: function(){
			...
			Ctrl.hostip = Ctrl.getParam("hostip");
			Ctrl.instancename = Ctrl.getParam("name");

			if(!Ctrl.hostip && !Ctrl.instancename){
				Messenger().post({
				  	message: '没有云主机数据！',
				  	type:'error',
			...
			Ctrl.metricUrl = DATASERVER + "/vmware/instance/" + Ctrl.hostip + "/metrics/?ins_name=" + Ctrl.instancename + "&metric=";
		...
		getInstanceInfo :function() {
			return $.get(DATASERVER + "/vmware/instance/" + Ctrl.hostip + "/info/?ins_name=" + Ctrl.instancename).fail(function(xhr){
				Messenger().post({
