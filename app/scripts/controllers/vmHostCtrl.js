define(['TM','echarts','eChartTheme',
	...
	var Ctrl = {
		init: function () {
			...
			Ctrl.basicUrl = DATASERVER + "/vmware/host/" + Ctrl.hostIp + "/";
		...
		bindEvent: function () {
			$('#host-instance-list-table').on('click', 'tbody > tr', function (event) {
				var $el = $(this);
				var name = $el.data('id');
				var url = 'vmInstance.html?name=' + name + "&hostip=" + Ctrl.hostIp;
		...
		getHostInstanceList: function () {
			return $.get(DATASERVER + "/vmware/instance/" + Ctrl.hostIp + "/ins_list/").fail(function (xhr) {
				Messenger().post({
